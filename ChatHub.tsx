  ...
  const composer = (
    <div className="border-t border-violet-500/10 pt-2">
      ...
      <div className="flex gap-2">
        <Input value={replyText} onChange={event => setReplyText(event.target.value)} placeholder={selectedCustomerId ? "พิมพ์ตอบลูกค้า…" : "ไม่พบ Customer PSID ของห้องนี้"} disabled={!selectedCustomerId} className="border-violet-500/15 bg-black/25 text-white placeholder:text-violet-100/25" onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendCurrentReply(); } }} />
        <Button aria-label="ส่งข้อความ" disabled={sendReply.isPending || !selectedCustomerId || (!replyText.trim() && !replyImageUrl.trim() && !stickerId.trim())} onClick={sendCurrentReply} className="shrink-0 rounded-xl bg-gradient-to-r from-fuchsia-600 to-violet-600"><Send className="h-4 w-4" /></Button>
      </div>
      <div className="mt-1 flex flex-wrap items-center gap-2">
        <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp" className="hidden" onChange={event => { handleImageUpload(event.target.files?.[0]); event.currentTarget.value = ""; }} />
        <Button type="button" size="sm" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={uploadImage.isPending} className="h-7 border-violet-500/15 bg-black/20 text-[10px] text-violet-100/70"><ImagePlus className="mr-1 h-3.5 w-3.5" />{uploadImage.isPending ? "กำลังอัปโหลด…" : "แนบรูป"}</Button>
        {replyImageUrl ? <span className="max-w-40 truncate text-[10px] text-emerald-300">รูปพร้อมส่ง</span> : null}
        <Input value={stickerId} onChange={event => setStickerId(event.target.value)} placeholder="Sticker ID (ถ้ามี)" className="h-7 min-w-32 flex-1 border-violet-500/10 bg-black/20 text-xs text-white placeholder:text-violet-100/20" />
        <span className="whitespace-nowrap text-[10px] text-violet-100/25">Meta</span>
        <Button type="button" size="sm" variant="outline" onClick={() => simulate("text")} disabled={simulateSend.isPending || !selectedCustomerId} className="h-7 border-cyan-400/20 bg-cyan-400/[0.04] text-[10px] text-cyan-200">จำลองข้อความ</Button>
        <Button type="button" size="sm" variant="outline" onClick={() => simulate("image")} disabled={simulateSend.isPending || !selectedCustomerId} className="h-7 border-cyan-400/20 bg-cyan-400/[0.04] text-[10px] text-cyan-200">จำลองรูป</Button>
      </div>
      ...
      {sendReply.isError ? <div className="mt-2 rounded-xl border border-amber-400/25 bg-amber-950/20 p-3 text-[11px] leading-5 text-amber-200">{isThreadControlConflict(sendReply.error.message) ? <><p className="font-semibold text-amber-100">Meta ไม่อนุญาตให้ส่งในเธรดนี้</p><p className="mt-1 text-amber-200/75">มีแอปอื่นถือสิทธิ์ควบคุมเธรดนี้อยู่ จึงไม่ควรกดส่งซ้ำ ระบบเก็บข้อความไว้ในช่องพิมพ์แล้ว</p></> : `ส่งไม่สำเร็จ: ${sendReply.error.message}`}</div> : null}
    </div>
  );

  const conversation = selected ? (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      <div className="flex items-center justify-between gap-3 border-b border-violet-500/10 px-5 py-3">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-500/25 to-violet-500/25 text-fuchsia-200"><CircleUserRound className="h-5 w-5" /></div>
          <div className="min-w-0">
            <h2 className="truncate text-base font-semibold text-white">{selected.customerName || "ลูกค้าใหม่"}</h2>
            <div className="mt-1 flex items-center gap-2 text-[11px] text-violet-100/40"><span className="truncate">{selected.pageName}</span><span>·</span><span>{timeLabel(selected.latestAt)}</span></div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Badge className="hidden border border-emerald-400/25 bg-emerald-400/10 text-emerald-300 sm:inline-flex">● ONLINE</Badge>
          <Button size="sm" variant="outline" onClick={confirmSelectedOrder} disabled={selectedConfirmed || confirmOrder.isPending} className="rounded-xl border-cyan-400/25 bg-cyan-400/10 text-xs text-cyan-200 hover:bg-cyan-400/20">{selectedConfirmed ? <><CheckCircle2 className="mr-1 h-3.5 w-3.5" />ยืนยันแล้ว</> : confirmOrder.isPending ? "กำลังบันทึก…" : "ยืนยันเป็นออเดอร์"}</Button>
          <Button size="sm" variant="outline" onClick={() => setOrderDetailOpen(true)} className="rounded-xl border-amber-400/25 bg-amber-400/10 text-xs text-amber-200 hover:bg-amber-400/20">ออเดอร์ ({linkedOrdersQuery.data?.length ?? 0})</Button>
          <Button size="sm" onClick={openOrderSummary} className="rounded-xl bg-fuchsia-600/80 text-xs hover:bg-fuchsia-500">สรุปออเดอร์</Button>
          {isMobile ? <button onClick={() => setMobileOpen(false)} className="rounded-xl p-2 text-violet-100/55 hover:bg-violet-500/10 hover:text-white" aria-label="ปิดห้องแชท"><X className="h-5 w-5" /></button> : null}
        </div>
      </div>
      ...
      <div className="border-t border-violet-500/10 bg-[#100d15] p-4">
        <div className="mb-3 flex flex-wrap items-center gap-1.5"><span className="mr-1 text-[10px] uppercase tracking-wider text-violet-100/35">ดูข้อความ</span>{([ ["all", "ทั้งหมด"], ["customer", "ลูกค้า"], ["page", "เพจ"] ] as const).map(([value, label]) => <button key={value} onClick={() => setMessageFilter(value)} className={`rounded-lg px-2.5 py-1 text-[10px] transition ${messageFilter === value ? "bg-fuchsia-500/20 text-fuchsia-200 ring-1 ring-fuchsia-400/30" : "text-violet-100/45 hover:bg-violet-500/10 hover:text-violet-100"}`}>{label}</button>)}</div>
        {summary ? <div className="mb-3 rounded-2xl border border-fuchsia-400/20 bg-fuchsia-500/[0.06] p-3"><div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold text-fuchsia-200">พรีวิวใบสรุป · {summary.orderNumber}</p><button onClick={() => setSummary(null)} className="text-[10px] text-violet-100/40 hover:text-white">ล้าง</button></div><p className="mt-1 text-[11px] text-violet-100/45">ตรวจข้อความด้านล่างให้ถูกต้องก่อนกดส่งให้ลูกค้า</p><Button variant="outline" size="sm" onClick={copySummary} className="mt-2 h-7 border-fuchsia-400/20 bg-transparent text-[10px] text-fuchsia-200">{copied ? <Check className="mr-1 h-3 w-3" /> : <Clipboard className="mr-1 h-3 w-3" />}{copied ? "คัดลอกแล้ว" : "คัดลอกข้อความสรุป"}</Button></div> : null}
        {composer}
      </div>
    </div>
  ) : <div className="flex h-full items-center justify-center text-sm text-violet-100/35">เลือกห้องแชทเพื่อเริ่มดูข้อความ</div>;

  return (
    <div className="min-h-[calc(100vh-2rem)] bg-[#09070d] text-white">
      <div className="mx-auto max-w-[1600px] space-y-5 p-2 sm:p-4 lg:p-6">
        <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-violet-500/15 bg-[#100c19] px-4 py-2 text-[11px] text-violet-100/55">
          <span className="inline-flex items-center gap-1.5 text-emerald-300"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />SYNC OK</span>
          <span>รอบล่าสุด: {threadsQuery.dataUpdatedAt ? new Date(threadsQuery.dataUpdatedAt).toLocaleString("th-TH", { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : "กำลังโหลด"}</span>
          <span className="text-violet-100/20">·</span>
          <span><Users className="mr-1 inline h-3 w-3" />{totalMessages.toLocaleString("th-TH")} ข้อความจากตารางกลาง</span>
          <span className="text-violet-100/20">·</span>
          <span className={metaErrorsQuery.data?.length ? "text-amber-300" : "text-emerald-300"}>Meta Error {metaErrorsQuery.data?.length ?? 0}</span>
          <span className="ml-auto text-violet-100/30">n8n API รอบละ 1 นาที</span>
        </div>

        {threadsQuery.isError ? (
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-red-400/20 bg-red-400/[0.06] px-4 py-3 text-xs text-red-200">
            <span>โหลดห้องแชทไม่สำเร็จ: {threadsQuery.error.message}</span>
            <Button size="sm" variant="outline" onClick={() => threadsQuery.refetch()} className="border-red-300/20 bg-transparent text-red-100">ลองใหม่</Button>
          </div>
        ) : null}

        <header className="relative overflow-hidden rounded-3xl border border-violet-500/20 bg-[#100c19] px-6 py-7 shadow-2xl shadow-violet-950/20 sm:px-8">
          <div className="pointer-events-none absolute -right-24 -top-36 h-80 w-80 rounded-full bg-fuchsia-600/20 blur-3xl" />
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="mb-4 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.28em] text-fuchsia-300"><Sparkles className="h-4 w-4" /> NIGHTOPS · PAGE CHAT HUB</div>
              <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">กล่องข้อความรวม</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-violet-100/55">กรองห้องตามเพจ สถานะอ่าน และออเดอร์ได้จากแถบด้านซ้าย</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Badge className="border border-emerald-400/30 bg-emerald-400/10 text-emerald-300"><span className="mr-2 h-1.5 w-1.5 rounded-full bg-emerald-400" /> LIVE API · 60s</Badge>
              <Button variant="outline" size="sm" onClick={() => threadsQuery.refetch()} className="border-violet-400/20 bg-violet-400/5 text-violet-100 hover:bg-violet-400/10"><RefreshCw className={`mr-2 h-3.5 w-3.5 ${threadsQuery.isFetching ? "animate-spin" : ""}`} /> รีเฟรช</Button>
            </div>
          </div>
        </header>

        <div className="grid gap-5 lg:grid-cols-[340px_minmax(0,1fr)]">
          <Card className="overflow-hidden rounded-3xl border-violet-500/15 bg-[#100d15] shadow-xl shadow-violet-950/10">
            <CardHeader className="border-b border-violet-500/10 pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base">แชทส่วนตัว <span className="ml-1 text-xs font-normal text-violet-100/35">{filteredThreads.length}</span></CardTitle>
                <button onClick={() => setPageFilterOpen(value => !value)} className={`flex items-center gap-1 rounded-xl px-2.5 py-1.5 text-[11px] ${selectedPages.length ? "bg-fuchsia-500/15 text-fuchsia-200" : "text-violet-100/45 hover:bg-violet-500/10 hover:text-violet-100"}`}><span className="h-2 w-2 rounded-sm border border-current" />เพจ{selectedPages.length ? ` · ${selectedPages.length}` : ""}<ChevronDown className="h-3 w-3" /></button>
              </div>
              <div className="relative mt-3">
                <Search className="absolute left-3 top-2.5 h-4 w-4 text-violet-100/30" />
                <Input value={search} onChange={event => setSearch(event.target.value)} placeholder="ค้นหาชื่อลูกค้าหรือข้อความ…" className="rounded-xl border-violet-500/15 bg-black/20 pl-9 text-white placeholder:text-violet-100/25 focus-visible:ring-fuchsia-400" />
                {pageFilterOpen ? <div className="absolute right-0 top-12 z-25 w-64 rounded-2xl border border-violet-400/20 bg-[#181020] p-3 shadow-2xl shadow-black/50"><div className="mb-2 flex items-center justify-between"><p className="text-[10px] font-semibold uppercase tracking-wider text-fuchsia-300">เลือกเพจ</p><button onClick={() => setSelectedPages([])} className="text-[10px] text-violet-100/40 hover:text-white">ล้าง</button></div>{availablePages.map(([pageId, pageName]) => <label key={pageId} className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-xs text-violet-50/80 hover:bg-violet-500/10"><input type="checkbox" checked={selectedPages.includes(pageId)} onChange={event => setSelectedPages(current => event.target.checked ? [...current, pageId] : current.filter(item => item !== pageId))} className="accent-fuchsia-500" /> <span className="truncate">{pageName}</span></label>)}{!availablePages.length ? <p className="p-2 text-xs text-violet-100/35">ยังไม่มีรายชื่อเพจ</p> : null}</div> : null}
              </div>
              <div className="mt-3 flex flex-wrap gap-1.5">
                {([ ["all", "ทั้งหมด"], ["unread", "ยังไม่อ่าน"], ["ordered", "มีออเดอร์"] ] as const).map(([value, label]) => <button key={value} type="button" onClick={() => setStatusFilter(value)} className={`rounded-lg px-2.5 py-1 text-[10px] transition ${statusFilter === value ? "bg-amber-400/15 text-amber-200 ring-1 ring-amber-300/25" : "text-violet-100/45 hover:bg-violet-500/10 hover:text-violet-100"}`}>{label}</button>)}
              </div>
            </CardHeader>
            <CardContent className="max-h-[calc(100dvh-18rem)] overflow-y-auto overscroll-contain p-2 [-webkit-overflow-scrolling:touch] lg:max-h-[650px]">
              {threadsQuery.isLoading ? <div className="flex items-center justify-center gap-2 p-12 text-sm text-violet-100/40"><RefreshCw className="h-4 w-4 animate-spin" />กำลังโหลดห้อง…</div> : filteredThreads.length === 0 ? <div className="p-12 text-center text-sm text-violet-100/40"><MessageCircle className="mx-auto mb-3 h-8 w-8 text-violet-100/15" />ยังไม่มีห้องตามตัวกรอง</div> : filteredThreads.map((thread, index) => <button key={thread.key} onClick={() => { setSelectedKey(thread.key); if (isMobile) setMobileOpen(true); }} className={`relative mb-1 flex w-full items-start gap-3 rounded-2xl p-3.5 text-left transition ${selected?.key === thread.key ? "bg-fuchsia-500/[0.10] ring-1 ring-fuchsia-400/35" : "hover:bg-violet-400/[0.06]"}`}><span className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${index % 2 ? "bg-violet-500/20 text-violet-200" : "bg-fuchsia-500/20 text-fuchsia-200"}`}>{(thread.customerName || "ลูกค้า").slice(0, 1)}</span><span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-2"><span className="truncate text-sm font-semibold text-violet-50">{thread.customerName || "ลูกค้าใหม่"}</span><span className="shrink-0 text-[10px] text-violet-100/35">{timeLabel(thread.latestAt)}</span></span><span className="mt-1 flex items-center gap-1.5 truncate text-[10px] text-fuchsia-200/55">{thread.unread ? <span className="rounded bg-amber-400/15 px-1 text-amber-200">ใหม่</span> : null}{thread.orderCount > 0 ? <span className="rounded bg-cyan-400/15 px-1 text-cyan-200">ออเดอร์ ({thread.orderCount})</span> : null}<span className="truncate">{thread.pageName}</span></span><p className="mt-1 truncate text-xs text-violet-100/50">{thread.preview || "ไม่มีข้อความล่าสุด"}</p></span></button>)}
            </CardContent>
          </Card>

          <Card className="overflow-hidden rounded-3xl border-violet-500/15 bg-[#100d15] shadow-xl shadow-violet-950/10 min-h-[600px] flex flex-col">
