...
export default function ChatHub() {
  ...
  const composer = <div className="border-t border-orange-500/10 pt-2">
    ...
    <div className="flex gap-2"><Input value={replyText} onChange={event => setReplyText(event.target.value)} placeholder={selectedCustomerId ? "พิมพ์ตอบลูกค้า…" : "ไม่พบ Customer PSID ของห้องนี้"} disabled={!selectedCustomerId} className="border-orange-500/15 bg-black/25 text-white placeholder:text-orange-100/25" onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendCurrentReply(); } }} /><Button aria-label="ส่งข้อความ" disabled={sendReply.isPending || !selectedCustomerId || (!replyText.trim() && !replyImageUrl.trim() && !stickerId.trim())} onClick={sendCurrentReply} className="shrink-0 rounded-xl bg-amber-500 text-[#1c1302]"><Send className="h-4 w-4" /></Button></div>
